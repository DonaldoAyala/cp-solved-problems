...
class Player:
	...
	def move(self, current_cell):
		...
		if key[pygame.K_LEFT]:
			if ( current_cell.walls[3] and self.pos_x < current_cell.col ):
				self.pos_x = current_cell.col
			else:
				self.go_left()
		if key[pygame.K_RIGHT]:
			if ( current_cell.walls[1] and self.pos_x + self.size > current_cell.col + current_cell.cell_size ):
				self.pos_x = current_cell.col + current_cell.cell_size - self.size
			else:
				self.go_right()
		if key[pygame.K_DOWN]:
			if ( current_cell.walls[2] and self.pos_y + self.size > current_cell.row + current_cell.cell_size ):
				self.pos_y = current_cell.row + current_cell.cell_size - self.size
			else:
				self.go_down()
		if key[pygame.K_UP]:
			if ( current_cell.walls[0] and self.pos_y < current_cell.row ):
				self.pos_y = current_cell.row
			else:
				self.go_up()
		...
		for i in range(maze.rows):
			for j in range(maze.columns):
				...
				if maze.cells[i][j].walls[0]:
					pygame.draw.line(self.window,color.white, (maze.cells[i][j].col * maze.cell_size,maze.cells[i][j].row * maze.cell_size), (maze.cells[i][j].col * maze.cell_size + maze.cell_size, maze.cells[i][j].row * maze.cell_size))
